...
// c++ -lm -o makeMatchedTree_IO makeMatchedTree_IO.cpp -L/cvmfs/cms.cern.ch/slc7_amd64_gcc700/external/boost/1.67.0/lib `root-config --glibs --cflags` -lTreePlayer -lboost_program_options
...
std::pair<int, int> findBest(
    ...
    std::vector<int>* itoskip = nullptr,
    std::vector<bool>* acceptmask = nullptr)
{
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()
        ...
        // optional
        ("dRmax"         , po::value<float>()->default_value(99999.), "max deltaR for track-genmu matching ")
        ("maxEvts"       , po::value<int>()->default_value(-1),       "max events to run (-1 for all) ")
    ...
    // copy values
    string filelist   = opts["input"].as<string>();
    string outputname = opts["output"].as<string>();

    cout << "... running on  : " << filelist << endl;
    ...
    // float dRmax = 0.3;
    // float dRmax = 99999.;
    // if (opts.count("dRmax")) dRmax = opts["dRmax"].as<float>();
    float dRmax = opts["dRmax"].as<float>();
    cout << "... max dR for TT-gen match: " << dRmax << endl;
    // float dRmax = 0.1;
    // float dRmax = 0.01;

    bool checkTToverlap = opts["check-overlap"].as<bool>();
    cout << "... no multiple TT match for the same gen muon: " << std::boolalpha << checkTToverlap << std::noboolalpha << endl;

    bool require_single_mu_in_endcap = opts["require-single-mu-in-endcap"].as<bool>();
    cout << "... require a single mu in the endcap: " << std::boolalpha << require_single_mu_in_endcap << std::noboolalpha << endl;
    ...
    cout << "... max events to run: " << maxEvts << endl;
    ...
    */
    ...
    tOut->Branch("barrel_pt",     &barrel_pt);
    tOut->Branch("barrel_eta",    &barrel_eta);
    tOut->Branch("barrel_theta",  &barrel_theta);
    tOut->Branch("barrel_phi",    &barrel_phi);
    tOut->Branch("barrel_charge", &barrel_charge);
    tOut->Branch("barrel_mult",   &barrel_mult);
    //
    tOut->Branch("ovrlap_pt",     &ovrlap_pt);
    tOut->Branch("ovrlap_eta",    &ovrlap_eta);
    tOut->Branch("ovrlap_theta",  &ovrlap_theta);
    tOut->Branch("ovrlap_phi",    &ovrlap_phi);
    ...
    for (uint iEv = 0; true; ++iEv)
    {
        ...
        if (!single_mu_in_endcap(mtkt) && require_single_mu_in_endcap)
            continue;
        ...
        // loop on gen muons
        std::vector<int> i_trk_to_skip;

        for (uint igm = 0; igm < *(mtkt.n_gen_mu); ++igm)
        {
            float eta = mtkt.gen_mu_eta.At(igm);
            float aeta = std::abs(eta);
            if ((aeta < fiducial_eta_min || aeta > fiducial_eta_max) && restrict_to_endcap_muons)
                continue;
            ...
            // build the track quality cuts for trk preselection
            std::vector<bool> trk_pass_qual (*mtkt.n_L1TT_trk);
            for (uint itrk = 0; itrk < *mtkt.n_L1TT_trk; ++itrk)
            {
                float chisq  = mtkt.L1TT_trk_chi2.At(itrk);
                int   nstubs = mtkt.L1TT_trk_nstubs.At(itrk);
