...
def dump_to_file(txtfilename, varslist, quantileslist, dtset, groups, binningx, binningy):
    """ write the quantiles found in a txt file for successive usage """
    print "... writing the results to", txtfilename

    result = collections.OrderedDict()

    result['binningx']  = binningx.tolist()
    result['binningy']  = binningy.tolist()
    result['quantiles'] = list(quantileslist)
    result['variables'] = list(varslist)

    for var in varslist:
        print '... saving var', var
        result[var] = collections.OrderedDict()
        for q in quantileslist:
            print '... ... saving quantile', q
            result[var][q] = collections.OrderedDict()
            for ibinx in range(len(binningx)-1):
                for ibiny in range(len(binningy)-1):
                    val = None
                    nev = 0
                    if (ibinx,ibiny) in groups.indices: ## only for non-empty bins
                        keygroup = groups.indices[ibinx,ibiny]
                        val = dtset.loc[ :, var].iloc[keygroup].quantile(q) # note: I have to index this as a str(tuple) because tuple is not a good key to json
                        nev = dtset.loc[ :, var].iloc[keygroup].size
                    result[var][q][str((ibinx, ibiny))] = collections.OrderedDict()
                    result[var][q][str((ibinx, ibiny))]['val'] = val
                    result[var][q][str((ibinx, ibiny))]['nev'] = nev
...
# filename = 'matchedTree_MuGun_PU0_EMTFpp.root'

filename = 'matchedTree_MuGun_PU0_alldetector.root'
...
region = 'endcap' # barrel, overlap, endcap
...
if region == 'endcap':
    data_format = data_format + (
        ...
        ('emtf_phi'         , None),
        ('emtf_charge'      , int),
        ('abs(emtf_eta)/emtf_eta' , int),
    )

    pt_name      = 'emtf_pt'
    aeta_name    = 'abs(emtf_eta)'
    atheta_name  = 'abs(emtf_theta)'
    theta_name   = 'emtf_theta'
    phi_name     = 'emtf_phi'
    charge_name  = 'emtf_charge'
    etasign_name = 'abs(emtf_eta)/emtf_eta'

    bins_trk_pt  = np.linspace(start = 2.0, stop = 202.0, num = 200+1)
    bins_trk_eta = np.linspace(start = 1.2, stop = 2.4,   num = 10+1)

    txtoutput = 'matching_windows_endcap.json'

elif region == 'overlap':
    data_format = data_format + (
        ('ovrlap_pt'          , None),
        ('abs(ovrlap_eta)'    , None),
        ('abs(ovrlap_theta)'  , None),
        ('ovrlap_theta'       , None),
        ('ovrlap_phi'         , None),
        ('ovrlap_charge'      , int),
        ('abs(ovrlap_eta)/ovrlap_eta' , int),
    )

    pt_name      = 'ovrlap_pt'
    aeta_name    = 'abs(ovrlap_eta)'
    atheta_name  = 'abs(ovrlap_theta)'
    theta_name   = 'ovrlap_theta'
    phi_name     = 'ovrlap_phi'
    charge_name  = 'ovrlap_charge'
    etasign_name = 'abs(ovrlap_eta)/ovrlap_eta'

    bins_trk_pt  = np.linspace(start = 2.0, stop = 202.0, num = 200+1)
    bins_trk_eta = np.linspace(start = 0.8, stop = 1.2,   num = 5+1)

    txtoutput = 'matching_windows_overlap.json'

elif region == 'barrel':
    data_format = data_format + (
        ('barrel_pt'          , None),
        ('abs(barrel_eta)'    , None),
        ('abs(barrel_theta)'  , None),
        ('barrel_theta'       , None),
        ('barrel_phi'         , None),
        ('barrel_charge'      , int),
        ('abs(barrel_theta)/barrel_theta' , int), ## use theta, so it will never be 0
    )

    pt_name      = 'barrel_pt'
    aeta_name    = 'abs(barrel_eta)'
    atheta_name  = 'abs(barrel_theta)'
    theta_name   = 'barrel_theta'
    phi_name     = 'barrel_phi'
    charge_name  = 'barrel_charge'
    etasign_name = 'abs(barrel_theta)/barrel_theta'

    bins_trk_pt  = np.linspace(start = 2.0, stop = 202.0, num = 200+1)
    bins_trk_eta = np.linspace(start = 0.0, stop = 0.8,   num = 10+1)

    txtoutput = 'matching_windows_barrel.json'
...
good_muon = dataframe[pt_name] > 0
good_trk  = dataframe['trk_pt'] > 0



# dummy_cut = dataframe['trk_pt'] > -999999
dataframe = dataframe[good_muon & good_trk].copy()
print dataframe
print dataframe.shape
...
# col_dphicharge = dataframe.loc[ : , 'emtf_phi'].subtract(dataframe.loc[ : , 'trk_phi']).multiply(dataframe.loc[ : , 'trk_charge']).apply(to_mpi_pi)
# col_dthetaendc = dataframe.loc[ : , 'emtf_theta'].subtract(dataframe.loc[ : , 'trk_theta']).multiply(dataframe.loc[ : , 'abs(emtf_eta)/emtf_eta'])

col_dphi   = dataframe.loc[ : , phi_name].subtract(dataframe.loc[ : , 'trk_phi']).apply(to_mpi_pi).abs()
col_deta   = dataframe.loc[ : , aeta_name].subtract(dataframe.loc[ : , 'abs(trk_eta)']).abs()
col_dtheta = dataframe.loc[ : , atheta_name].subtract(dataframe.loc[ : , 'abs(trk_theta)']).abs()
col_dR     = np.sqrt((col_dphi*col_dphi)+(col_deta*col_deta)) # mega figo! realizza a catena tutte le operazioni
# col_dphicharge = dataframe.loc[ : , phi_name].subtract(dataframe.loc[ : , 'trk_phi']).multiply(dataframe.loc[ : , 'trk_charge']).apply(to_mpi_pi)

### make (trk - emtf) * charge_trk so that it is positive
...
col_ptresol    = dataframe.loc[ : , pt_name].divide(dataframe.loc[ : , 'trk_pt'])
...
dataframe.loc[:, 'delta_phicharge'] = col_dphicharge
dataframe.loc[:, 'delta_thetaendc'] = col_dthetaendc
dataframe.loc[:, 'pt_resol']        = col_ptresol
print '... appended'
...
if not dump_to_histo:
    dump_to_file(
        txtfilename   = txtoutput,
        varslist      = ['delta_phi', 'delta_theta', 'delta_eta', 'delta_R', 'delta_phicharge', 'delta_thetaendc', 'pt_resol'],
        quantileslist = [quantile_l99, quantile_l, quantile_l90, quantile_c, quantile_h90, quantile_h, quantile_h99],
        dtset = dataframe,
        groups = gpb,
        binningx = bins_trk_pt,
        binningy = bins_trk_eta,
    )
    ...
    fOut = ROOT.TFile('matching_windows_test.root', 'recreate')
    ...
    for ix in range(len(bins_trk_pt)-1):
        for iy in range(len(bins_trk_eta)-1):
            ...
            if (ix,iy) not in gpb.indices: ## to skip empty bins
                print ".......", (ix,iy) , "is empty, skipping"
                continue
    ...
    if True:
        ...
        tIn_xc  = uproot.open(filename)['tree']
        keys = tIn_xc.keys()
