...
for sf in sfs:
    ...
    # fIn = ROOT.TFile.Open('../matched_tree_MuMu_flatPt_0PU_sf_%s.root' % str(sf))
    ...
    eff_upgtkmu = make_eff(tIn, x_var, '', 'upgtkmu_pt > %f' % float(threshold),  '%i, %f, %f' % (nbins, xmin, xmax), 'eff_upgtkmu_%s' % str(sf))
...
ttext = ROOT.TLatex(0.3, 0.2, "p_{T}^{tkmu} > %.0f GeV" % threshold)
ttext.SetNDC(True)
...
c1.Print('efficiencies/eff_plot_sfcompare.pdf', 'pdf')
